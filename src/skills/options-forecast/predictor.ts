...
import { MODELS, getOpenRouterHeaders, OPENROUTER_CONFIG } from '../../config/models';
...
function aggregateSignals(
...
): { overall: 'bullish' | 'bearish' | 'neutral'; score: number; confidence: number } {
  if (signals.length === 0) {
  ...
  for (const signal of signals) {
    const directionValue =
      signal.interpretation === 'bullish' ? 1 : signal.interpretation === 'bearish' ? -1 : 0;
...
${extraContext ? `\nCMC OPTIONS-POSITIONING ANALYSIS (proxy context from CoinMarketCap):\n${extraContext}\n` : ''}
...
Respond in this exact JSON format:
{
  ...
  try {
    ...
    throw new Error(`Failed to get API headers: ${e instanceof Error ? e.message : 'Unknown error'}`);
  }

  console.log(`[PricePredictor] Calling OpenRouter with model: ${MODELS.reasoning}`);
  ...
  if (!response.ok) {
    ...
    console.error(`[PricePredictor] OpenRouter API error: ${response.status} - ${errorBody}`);
  ...
  if (!content) {
    ...
    throw new Error(`No response from LLM. Model: ${MODELS.reasoning}. Response: ${JSON.stringify(result)}`);
  }

  console.log(`[PricePredictor] LLM response received (${content.length} chars)`);

  // Parse JSON from response
  const jsonMatch = content.match(/\{[\s\S]*\}/);
  if (!jsonMatch) {
    console.error(`[PricePredictor] Could not parse JSON from response: ${content.substring(0, 200)}`);
  ...
  try {
    ...
    console.error(`[PricePredictor] JSON parse error: ${parseError}. Content: ${jsonMatch[0].substring(0, 200)}`);
    throw new Error(`Failed to parse LLM response as JSON: ${parseError}`);
  }

  // Calculate threshold if not provided - use a round number near spot price
  const threshold = parsed.threshold || Math.round(data.spotPrice / 100) * 100;

  // Determine conclusion based on direction if not explicitly provided
  const conclusion = parsed.conclusion || (parsed.direction === 'down' ? 'BELOW' : 'ABOVE');
...
export async function generateForecast(
  ...
  const expectedVolatility =
    (data.iv.current / 100) * Math.sqrt(config.hours / 8760) * config.volatilityMultiplier;
